...
func connLoop(conn *net.TCPConn) {
	...
	for scanner.Scan() {
		bmpMsg, err := bmp.ParseBMPMessage(scanner.Bytes())
		if err != nil {
			log.Info(err)
			continue
		}
		typ := bmpMsg.Header.Type
		peerAddr := bmpMsg.PeerHeader.PeerAddress
		t1, t2 := math.Modf(bmpMsg.PeerHeader.Timestamp)
		t2 = math.Ceil(t2 * math.Pow10(6))
		t := time.Unix(int64(t1), int64(t2))
